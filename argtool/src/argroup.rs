...
impl ArgGroup {
    ...
    pub fn parse(
            ...
            match sys_args.next() {
                ...
                Some(arg) => {
                    ...
                    // For pos type argument if available -> not start with - , not coupled argument
                        ...
                        self.arg_map
                            .insert(argitem.name.to_string(), ArgValue::STR(arg.to_string()));
                        cc += 1;
        ...
        if cc != pos_num {
            return Err(anyhow!(
                "Not enough positional argument, expect {}, got {}",
                pos_num,
                cc
            ));
        }
...
impl ArgGroup {
    ...
    fn match_arg(
        &mut self,
        keyname: String,
        sys_args: &mut impl Iterator<Item = String>,
    ) -> Result<(), anyhow::Error> {
        if keyname == "h" || keyname == "help" {
            println!("{}", self);
            process::exit(0x0100);
        }
        ...
        match arg_item.arg_type {
            ...
            ArgType::FlagType => {
                ...
                if arg_item.default == "true" {
                    ...
                    return Err(anyhow!(
                        "Flag argument --{} default value should be true / false, got {}",
                        &keyname,
                        &arg_item.default
                    ));
                }
                self.arg_map
                    .insert(keyname.to_string(), ArgValue::STR(flag));
            }

            ArgType::PositionalType => {
                return Err(anyhow!("PositionalType should not be in keyname match"))
            }
        };
        Ok(())
    }
            ...
            match self.arg_map.get(keyname) {
                ...
                None => {
                    ...
                    if let ArgType::ListType = argitem.arg_type {
                        self.arg_map
                            .insert(argitem.name.to_owned(), ArgValue::VEC(vec![]));
                    } else {
                        self.arg_map.insert(
                            argitem.name.to_owned(),
